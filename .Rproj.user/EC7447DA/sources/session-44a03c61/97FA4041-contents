# SCRIPT: load some real topographic and veg data for 
  # an arbitrary study area for use in generating simulated
  # data for an RSF model
...
## Download elevation data ----
    ...
    # use elevatr package: get_elev_raster    
...
# Set years
years_to_download <- 2015:2024
cat("Years:", paste(years_to_download, collapse = ", "), "\n\n")

## Get RAP data --------

   # use rapr package  (?get_rap)
...
# Change to ~90m resolution for faster processing
...
output_dir <- "RSF_SIM/mojave_spatialdata_forsim"
...
for(c in 1:length(cover_types)){
  ...
  writeRaster(this, 
              ...
              overwrite = TRUE)
  writeRaster(veg_stack_yr[[cover_types[c]]],
              file.path(output_dir, sprintf("%s_byyear.tif",cover_types[c])), 
              overwrite = TRUE)
}
