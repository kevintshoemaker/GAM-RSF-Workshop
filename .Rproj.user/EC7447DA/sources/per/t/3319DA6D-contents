...
pts_dir = "RSF_SIM/telemetry_data"
...
hr <- trk_grouped %>%
  mutate(hr = map(track, ~ hr_kde(.x, levels = 0.9, h=c(1000,1000), keep.data=T )))  
    ...
    # explore home ranges and observed/random
i=9
hr$ID[i]
...
write_csv(master_df,"RSF_SIM/master_count_df_500m.csv")
write_csv(transform_df,"RSF_SIM/transform_df_count_500m.csv")
...
ggplot(master_df,aes(x,y,col=res)) +   # plot residuals spatially and look for patterns
  geom_point()
...
# mod_tmb1  = glmmTMB(fm,
#                     ziformula = ~ ELEV_s,
#                     dispformula = ~ log(nids),   # among individual variaton
#                     data=master_df,
#                     family=nbinom2(link="log")
# )
# 
# summary(mod_tmb1)   # non-convergence issue
# 
...
qqnorm(p$resids);abline(a = 0, b = 1)    # looks okay

  # check spatial correlation in residuals

ggplot(p, aes(x, y, col = resids)) + scale_colour_gradient2() +     # some clustering remaining
  geom_point()
...
# Try standard logistic RSF --------

   ## generate random points within the study area

rand_points = st_as_sf( st_sample(mask_vector,10000) )
rand_points = cbind(rand_points,st_coordinates(rand_points))
rand_points$used = 0
head(rand_points)

used_points = dat[,c("ID")]
used_points = cbind(used_points,st_coordinates(used_points))
used_points$used = 1
head(used_points)

rand_points$ID = as.factor(sample(unique(used_points$ID),nrow(rand_points),replace=T))
head(rand_points)

rand_points = st_drop_geometry(rand_points)

used_points = st_drop_geometry(used_points)

all_points = rbind(used_points,rand_points[,names(used_points)])
all_points = st_as_sf(all_points,coords = c("X","Y"),crs=crs_proj, remove=F)

## extract covariates 

ext = terra::extract(cov_layers,vect(all_points))[,-1]
all_points = cbind(all_points,ext)

names(all_points)

all_points = st_drop_geometry(all_points)
all_points=all_points[complete.cases(all_points),]

# run logistic regression model ------

all_points$ELEV_s = scale(all_points$elevation)

if(max(all_points$X)>100000) all_points[,c("X","Y")] = all_points[,c("X","Y")]/1000
mesh = make_mesh(all_points,c("X","Y"), cutoff=5 )
plot(mesh)

## something seems a little strange here- due to the study area being defined by a buffer, there is an area at the edge of the study area that will have 0 points by definition- therefore the random field will always try to predict zeros at the edge. but this is just an artifact

## try to fit standard logistic regression with random slopes for individual
  #  note by= term used to allow among-individual variation
  #  could incorporate interactions... 

names(all_points)
fit2 <- sdmTMB(
  used ~ s(ELEV_s,k=4,bs="cr") + scale(slope) + scale(TREE) + scale(AFG) + 
    (scale(slope) + scale(AFG) + scale(TREE) | ID) ,    # s(ID,ELEV_s,bs="fs",k=4)  
  data = all_points,
  mesh = mesh,
  family = binomial(),   
  spatial = "on"
...
tidy(fit2,effects = "fixed",conf.int = T)
tidy(fit2, effects = "ran_pars",conf.int = T)

xx = seq(min(all_points$X),max(all_points$X),length=75)
yy= seq(min(all_points$Y),max(all_points$Y),length=75)

nd= expand.grid(X=xx*1000,Y=yy*1000)
nd = st_as_sf(nd,coords = c("X","Y"),crs=crs_proj, remove=F)

## extract covariates 

ext = terra::extract(cov_layers,vect(nd))[,-1]
nd=cbind(nd,ext)
nd=st_drop_geometry(nd)
nd$ID=unique(all_points$ID)[1]
if(max(nd$X)>100000) nd[,c("X","Y")] = nd[,c("X","Y")]/1000

nd$ELEV_s = scale(nd$elevation)
p <- predict(fit2, newdata=nd,  type="response")  

names(p)


ggplot(p, aes(X, Y, z = est_non_rf)) + geom_contour_filled()
# ggplot(p, aes(X, Y, z = plogis(est_rf))) + geom_contour_filled()
ggplot(p, aes(X, Y, z = est)) + geom_contour_filled() +
  geom_point(data=used_points,aes(X/1000,Y/1000),inherit.aes = FALSE,col="gray",alpha=0.05,pch=20)
...
p = predict(fit2)
p$resids <- residuals(fit2) # randomized quantile residuals
hist(p$resids)
...
res = dharma_residuals(s, fit2, return_DHARMa = T)  

testResiduals(res)
testZeroInflation(res)    # dispersion test failed
testSpatialAutocorrelation(res,all_points$X,all_points$Y)
...
this = ggpredict(fit2,"ELEV_s [all]")    # takes a while
plot(this)

this = ggpredict(fit2,"slope [0:60 by=5]")  
plot(this)
